...
public interface FieldInjectionVisitor extends InjectionVisitor {

    Optional<? extends Injection<FieldSite>> visitField(Lazy<FieldSite> site);

    @Override
    default Injection<?>[] visit(Lazy<ClassSite> site) {
        Lazy<FieldSite>[] fields = site.get().getFields();
        Injection<?>[] injections = new Injection[fields.length];
        int effectiveI = 0;
        for (Lazy<FieldSite> field : fields) {
            Optional<? extends Injection> injection = visitField(field);
